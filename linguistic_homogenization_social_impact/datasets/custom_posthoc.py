...
def posthoc_dunn(
    ...
    for i, j in combs:
        vs[i, j], zs[i, j] = compare_dunn(x_groups_unique[i], x_groups_unique[j])
    ...
    vs[tri_lower] = np.transpose(vs)[tri_lower]
    np.fill_diagonal(vs, 1)
    return DataFrame(vs, index=x_groups_unique, columns=x_groups_unique), DataFrame(
        zs, index=x_groups_unique, columns=x_groups_unique
    )
...
def posthoc_conover_friedman(
    ...
    T2 = 1.0 / S2 * np.sum((R - n * m * ((m * k + 1.0) / 2.0)) ** 2.0)
...
def posthoc_npm_test(
    ...
    def compare(m, u):
        a = [
            (ri.loc[groups[u]] - ri.loc[groups[_mi]])
            / (
                sigma
                / np.sqrt(2)
                * np.sqrt(1.0 / ni.loc[groups[_mi]] + 1.0 / ni.loc[groups[u]])
            )
            for _mi in m
        ]
        return np.array(a)
...
def posthoc_tukey_hsd(
    x: Union[list, np.ndarray, DataFrame], g: str, alpha: float = 0.05
) -> DataFrame:
