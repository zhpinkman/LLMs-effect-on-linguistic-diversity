...
void function_dynam(const double tstart, const double tend, size_t regime, const gsl_vector *xstart,
	...
	switch (regime) {
		case 0:
			gsl_vector_set(x_tend, 0, param[0] * gsl_vector_get(xstart, 0) + param[1] * gsl_vector_get(xstart, 1));
			gsl_vector_set(x_tend, 1, param[2] * gsl_vector_get(xstart, 1) + param[3] * gsl_vector_get(xstart, 0));
			break;
	}
}
...
void function_jacob_dynam(const double tstart, const double tend, size_t regime, const gsl_vector *xstart,
	...
	switch (regime) {
		case 0:
			gsl_matrix_set(Jx, 0, 0, param[0]);
			gsl_matrix_set(Jx, 0, 1, param[1]);
			gsl_matrix_set(Jx, 1, 0, param[3]);
			gsl_matrix_set(Jx, 1, 1, param[2]);
			break;
	}
}
...
void function_noise_cov(size_t t, size_t regime, double *param, gsl_matrix *y_noise_cov, gsl_matrix *eta_noise_cov){
	...
	gsl_matrix_set(eta_noise_cov, 0, 0, param[4]);
	gsl_matrix_set(eta_noise_cov, 1, 0, param[5]);
	gsl_matrix_set(eta_noise_cov, 0, 1, param[5]);
	gsl_matrix_set(eta_noise_cov, 1, 1, param[6]);

	gsl_matrix_set(y_noise_cov, 0, 0, param[7]);
	gsl_matrix_set(y_noise_cov, 1, 1, param[8]);
...
void function_initial_condition(double *param, gsl_vector **co_variate, gsl_vector **pr_0, gsl_vector **eta_0, gsl_matrix **error_cov_0, size_t *index_sbj){
	...
	gsl_matrix_set((error_cov_0)[regime], 0, 0, -9.21034037197618);
	gsl_matrix_set((error_cov_0)[regime], 1, 1, -9.21034037197618);
	}
	for(i=0; i < num_sbj; i++){
